...
validateattributes(b,{'double'},{'nonsparse'},mfilename,'B',1);

if all(b(:) == 0)
    error(message('images:ftrans2:expectAtLeastOneNonZeroElement','B'));
end

if nargin < 2,
    t = [1 2 1;2 -4 2;1 2 1]/8;
else
    validateattributes(t,{'double'},{'nonsparse'},mfilename,'T',2);
    if all(t(:)==0)
        error(message('images:ftrans2:expectAtLeastOneNonZeroElement','T'));
    end
...
n = (length(b)-1)/2;
if floor(n)~=n,
    error(message('images:ftrans2:bMustBeOddLength'))
end
if any(abs(b-rot90(b,2))>sqrt(eps)) || min(size(b))~=1,
    error(message('images:ftrans2:bMustBeSymmetric'))
end
