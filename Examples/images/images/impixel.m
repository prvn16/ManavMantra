...
validateattributes(a,{'uint8' 'uint16' 'double' 'logical' 'single' 'int16'}, ...
...
case 3
   ...
   varargout{3} = rgb;
otherwise
   error(message('images:impixel:invalidNumberOfArguments'))
end
...
if (nargin < 3),
    if (nargin == 0),
        ...
        [~,~,a,kind] = getimage;
        if ~kind, 
            error(message('images:impixel:imageNotFound'))
        end
        if kind==1, 
            cm = colormap(gca);
        else 
...
if length(xi) ~= length(yi)
    error(message('images:impixel:notSameSize'))
end
...
function checkindexedimage(a)
  ...
  if ndims(a)~=2 || isempty(a)
    error(message('images:impixel:invalidIndexedImage'))
  end
