...
[a, h, boundary, sameSize, convMode] = parse_inputs(args{:});
...
if (isSeparable(a, h)&& (numel(size(a))<=3))
  ...
  elseif (isequal(class(a),'double') && ismatrix(h) && numel(size(a))==3)
...
elseif (kernelIsReal && ~imageIsReal)
  ...
  realB = imfilter_mex(real(a), outSize, h, nonzero_h,...
                    conn, start, sameSize, convMode, ippFlag);

  imagB = imfilter_mex(imag(a), outSize, h, nonzero_h, ...
                    conn, start, sameSize, convMode, ippFlag);    
...
elseif (~kernelIsReal && imageIsReal)
  ...
  realB = imfilter_mex(a, outSize, real(h), real(nonzero_h),...
                    conn, start, sameSize, convMode, ippFlag);

  imagB = imfilter_mex(a, outSize, imag(h), imag(nonzero_h), ...
                    conn, start, sameSize, convMode, ippFlag);    
  ...
  realHrealA = imfilter_mex(real(a), outSize, real(h), real(nonzero_h),...
                    conn, start, sameSize, convMode, ippFlag);
  realHimagA = imfilter_mex(imag(a), outSize, real(h), real(nonzero_h),...
                    conn, start, sameSize, convMode, ippFlag);
  imagHrealA = imfilter_mex(real(a), outSize, imag(h), imag(nonzero_h),...
                    conn, start, sameSize, convMode, ippFlag);                
  imagHimagA = imfilter_mex(imag(a), outSize, imag(h), imag(nonzero_h), ...
                    conn, start, sameSize, convMode, ippFlag);
