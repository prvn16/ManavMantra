...
a = varargin{1};
validateattributes(a,{'numeric'},{},mfilename,'h',1);
...
switch nargin
  ...
  case 2
    ...
    switch numel(varargin{2})
      ...
      otherwise
        error(message('images:freqz2:with2Inputs2ndMustBe2ElemVectOrScalar'))
    end
  ...
  case 4
    ...
    if (numel(varargin{2}) == 1) && (numel(varargin{3}) == 1)
        ...
        switch numel(varargin{4})
          ...
          otherwise
            error(message('images:freqz2:fourthInputMustBe2ElemVectOrScalar'))
        end
        ...
        warning(message('images:freqz2:dxNotNeededWhenFxFyAreThere'))
        Fx = varargin{2};
        Fy = varargin{3};
        spacing = 'meshgrid';
    end
...
if (Dx <= 0) || (Dy <= 0)
    error(message('images:freqz2:DxDyMustBePositiveScalars'))
end

switch spacing
  case 'equal'
    if (Nx <= 0) || (Ny <= 0) || (floor(Nx) ~= Nx) || (floor(Ny) ~= Ny)
        error(message('images:freqz2:NxNyMustBePositiveIntegers'))
    end
    
  case 'meshgrid'
    if (min(numel(Fx)) ~= 1) || (min(numel(Fy)) ~= 1)
        if ~isequal(size(Fx), size(Fy))
            error(message('images:freqz2:ifFxFyNotVectorsThenMustBeSameSize'))
        end
    end
            
  otherwise
      error(message('images:freqz2:internalProblemSpacingNotSet'))
end
