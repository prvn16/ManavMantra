...
%   (objects) from a binary image where the attribute string or char vector
%   ATTRIB has values in RANGE, producing another binary image BW2. RANGE
%   is a 2-by-1 vector of low and high values. (The minimum and maximum
%   values are considered to be in the range.) Objects that do not meet the
%   criterion are removed. The default connectivity is 8.
%
...
switch numel(p)
case 1
    validateattributes(p, {'double'}, {'finite', 'row', 'nonsparse', 'integer', 'positive'}, ...
        mfilename, 'P', 3 + argOffset);
case 2
    validateattributes(p, {'numeric'}, {'row', 'nonsparse', 'real', 'nondecreasing'}, ...
        mfilename, 'P', 3 + argOffset);
otherwise
