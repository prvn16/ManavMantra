...
switch (meta.encoding)
 ...
 case {'gzip', 'gz'}
  ...
  c = onCleanup(@() fclose(fidTmp));
  tempFileCleaner = onCleanup(@() delete(tmpFile));
  ...
  fidTmpBase = fopen(tmpBase, 'rb');
  cleaner = onCleanup(@() fclose(fidTmpBase));
  
  meta.encoding = 'raw';
  data = readData(fidTmpBase, meta, datatype);
