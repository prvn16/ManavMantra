...
%   data.valueEdit.String = num2str(data.valueSlider.Value);
%
%   ... excerpt from the edit's callback ...
%
%   data = guidata(gcbo); % need handles, may need error info
%   val = str2double(data.valueEdit.String);
%   if isnumeric(val) & length(val)==1 & ...
%      val >= data.valueSlider.Min & ...
%      val <= data.valueSlider.Max
%     data.valueSlider.Value = val;
%   else
