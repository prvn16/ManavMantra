...
drawnow;
if ~isempty(internal.matlab.importtool.ImportableFileIdentifier.getSetImportWizardInstance) && ...
        ((nargin == 0) || ... % assume Import Wizard usage when no arguments are specified
        (~isempty(varargin{1}) && ... % file is specified and it uses the Import Wizard
        internal.matlab.importtool.ImportableFileIdentifier.usesImportWizard(localGetAbsolutePath(varargin{1}))))
    if ~isSynchronous
...
if useFileDialog
    ...
    if ctorFile.length>1e6 && useTextImportTool(isSynchronous, fileAbsolutePath)
...
if ~useClipboard && useTextImportTool(isSynchronous, fileAbsolutePath)
...
function state = useTextImportTool(isSynchronous, fileAbsolutePath)
    state = internal.matlab.importtool.ImportableFileIdentifier.useTextImportTool(...
        isSynchronous, fileAbsolutePath);
end
...
function ret = ishdf(file)
    ret = internal.matlab.importtool.ImportableFileIdentifier.useHDFTool(file);
end
