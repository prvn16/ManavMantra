classdef ToggleButtonGroupEditor < ...
	...
	methods
		...
		function props = getEditorState(this)
			...
			iconPaths = eval([this.dataType.Name, '.IconPaths']);
			
			% For all the icon paths... convert them to a base 64 URL
			urls = [];
			for idx = 1:length(iconPaths)
				iconPath = iconPaths{idx};
				urls = [urls appdesservices.internal.peermodel.convertImageToBase64URL(iconPath)]; %#ok<AGROW>
			end
