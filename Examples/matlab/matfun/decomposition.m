classdef (Sealed) decomposition < matlab.mixin.CustomDisplay & matlab.mixin.internal.Scalar
    ...
    methods
        ...
        function f = set.CheckCondition(f, doCheck)
            ...
            if doCheck
                if f.useRcond
                    if isempty(f.rcondSaved)
                        f.rcondSaved = rcond(f.Underlying);
                    end
                else
                    f.rankSaved = f.Underlying.rank_;
                    f.ranktolSaved = f.Underlying.ranktol_;
                end
            end
        end
        ...
        function isIllCond = isIllConditioned(f)
            ...
            if f.useRcond
                ...
                isIllCond = f.rankSaved < min(f.MatrixSize);
            end
        end
    ...
    methods (Access = private)
        function warnIfIllConditioned(f)
            if f.useRcond
                ...
                if f.rankSaved < min(f.MatrixSize)
                    warning(message('MATLAB:rankDeficientMatrix',sprintf('%d',f.rankSaved),sprintf('%13.6e',f.ranktolSaved)));
                end
            end
        end
    end
...
indType = strncmpi(args{1}, typeList, max(strlength(args{1}),1));
indName = strncmpi(args{1}, NameList, max(strlength(args{1}),1));

if nnz(indType) + nnz(indName) ~= 1
    % Let validatestring give the error message
    validatestring(args{1}, [typeList, NameList]);
end
...
if nnz(indType) == 1
    ...
    indName = strncmpi(args{2}, NameList, max(strlength(args{2}),1));
    
    if ismember(type, {'chol', 'ldl', 'triangular'})
        % Check for 'upper' / 'lower'
        indUpLo = strncmpi(args{2}, uploList, max(strlength(args{2}),1));
        
        if nnz(indName) + nnz(indUpLo) ~= 1
            % Let validatestring give the error message
            validatestring(args{2}, [uploList, NameList]);
        end
        ...
        if any(indUpLo)
            ...
            if length(args) >= 3
                indName = strncmpi(args{3}, NameList, max(strlength(args{3}),1));
            end
        end
    end
end
...
for ii=1:2:(length(args)-argOffset)
    ...
    if ii > 1
        ...
        indName = strncmpi(args{argOffset+ii}, NameList, max(strlength(args{argOffset+ii}),1));
    end
