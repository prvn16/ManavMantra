...
if ~images.internal.magPercentsDiffer(x,power_of_2_larger)
    power_of_2_larger = 2*power_of_2_larger;
elseif ~images.internal.magPercentsDiffer(x,power_of_2_smaller)
    power_of_2_smaller = power_of_2_smaller/2;
end

if ~images.internal.magPercentsDiffer(x,1)
    if zoomingIn
        y = 2;
    else
        y = 2/3;
...
if (x < in_between_mag) && images.internal.magPercentsDiffer(x,in_between_mag);
    y = in_between_mag;
