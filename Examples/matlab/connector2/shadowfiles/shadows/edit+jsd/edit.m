...
clientServiceRegistryFacade = ClientServiceRegistryFactory.getClientServiceRegistryFacade();
editorService = clientServiceRegistryFacade.getEditorService();
isMlxFile = com.mathworks.services.mlx.MlxFileUtils.isMlxFile(file);
fileName = java.io.File(file);
% If the file is a live script file and it does not exists.
if(isMlxFile && ~fileName.exists()) 
    opcPackage = com.mathworks.services.mlx.OpcUtils.createEmptyLiveScriptFile();
    com.mathworks.services.mlx.MlxFileUtils.write(fileName, opcPackage); 
end
...
elseif (strcmp(ext, ['.' mexext]))
    ...
    errID = 'MexFile';
elseif (strcmp(ext, '.mdl')) % MLSEDU doesn't support Simulink
    errMessage = sprintf('Can''t edit the MDL-file ''%s''.', s);
    errID = 'MdlFile';
elseif (strcmp(ext, '.mlapp'))
    error(message('MATLAB:connector:Platform:DependentFunctionNotSupported', 'edit', 'appdesigner'))
end
handleError(errMessage, errID);
