...
%   RESULT = XSLT(SOURCE,STYLE,DEST) transforms an XML document using a stylesheet
%   and returns the resulting document's URL.  The function uses these inputs, the
%   first of which is required:
%     SOURCE is the filename or URL of the source XML file.  SOURCE
%       can also be a DOM node.
%     STYLE is the filename or URL of an XSL stylesheet DEST is the filename or URL
%     of the desired output document; if
%       DEST is absent or empty, the function uses a temporary filename. If DEST is
%       '-tostring', the output document will be returned as a MATLAB character
%       vector.
%
%   [RESULT,STYLE] = XSLT(...) returns a processed stylesheet appropriate for passing
%   to subsequent XLST calls as STYLE. This prevents costly repeated processing of
%   the stylesheet.
%
%   XSLT(...,'-web') displays the resulting document in a Web Browser.
%
%   Example: This will convert a file "info.xml" to a temporary file using the
%   stylesheet "info.xsl" and launch the result in the help browser.  MATLAB has
%   several info.xml files which are used by the Start Button.
%
...
% Advanced use:
%   SOURCE can also be a XSLTInputSource STYLE can also be a StylesheetRoot or
%   XSLTInputSource DEST can also be an XSLTResultTarget.  Note that RESULT may be
%      empty in this case since it may not be possible to determine a URL.
%   ; if STYLE
%       is absent or empty, the function uses the stylesheet named in the
%       xml-stylesheet processing instruction in the SOURCE XML file.  (This does not
%       always work)
