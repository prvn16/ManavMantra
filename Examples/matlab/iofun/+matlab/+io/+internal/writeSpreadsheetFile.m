function writeSpreadsheetFile(t,filename,ext,varargin)
%WRITEXLSFILE Write a table to an Excel spreadsheet file.

%   Copyright 2017 The MathWorks, Inc.

[writeVarNames,writeRowNames,sheetName,range,locale] = getArgs(t,varargin);
% create workbook without live Excel instance
...
[height_t, width_t] = size(t);

% How many rows are we supposed to write.
if truncateRows && (row2 - row1 + 1 <= height_t)
...
if writeRowNames
    ...
    if writeVarNames
        rownames = [t.Properties.DimensionNames{1}; rownames];
    end
    vars = [vars rownames];
end
...
t_data = getVars(t);
fields = fieldnames(t_data);
t_Properties_VariableNames = t.Properties.VariableNames;
for j = 1:width_t
    if colCount > maxCol, break; end
    
    varnamej = t_Properties_VariableNames{j};
    
    if dataHeight > 0
        varj = t_data.(fields{j});
