...
elseif pathFormat == "cell"
    ...
    tails = tails(pred>0);
    if reverseDirection
        tree = digraph(tails, heads, G.EdgeProperties(edgepred(edgepred>0), :), G.NodeProperties);
    else
        tree = digraph(heads, tails, G.EdgeProperties(edgepred(edgepred>0), :), G.NodeProperties);
    end
