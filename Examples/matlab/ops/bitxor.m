...
%
%   C = BITXOR(A,B,ASSUMEDTYPE) assumes A and B are of type ASSUMEDTYPE.
%   If A and B are double arrays, then ASSUMEDTYPE can be 'int8', 'uint8',
%   'int16', 'uint16', 'int32', 'uint32', 'int64', or 'uint64' (the default).
%   For example, BITXOR(A,B,'int32') is equivalent to 
%   double(BITXOR(int32(A),int32(B))). All elements in A and B must have 
