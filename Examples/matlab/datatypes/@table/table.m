    ...
    methods
        ...
        %   See also READTABLE, CELL2TABLE, ARRAY2TABLE, STRUCT2TABLE.
            ...
            if nargin == 0
                ...
                [numVars, sz, vartypes, varnames,rownames,supplied] ...
                ...
                if supplied.Size % preallocate from specified size and var types
                    ...
                    % Throw a warning if data contains a scalar text that
                    % exactly matches a name-value pair name.
                    tabular.warnIfAmbiguousText(vars,numRows,pnames);
                    
                    % No NV pairs provided
                    if isequal(numVars,nargin)
...
%             end
    ...
    methods(Hidden, Static)
        function t = loadobj(s)            
            ...
            if isfield(s.props,'VersionSavedFrom') && s.props.VersionSavedFrom >= 2.1
