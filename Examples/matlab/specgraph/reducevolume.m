...
if numel(r)~=3 
  error(message('MATLAB:reducevolume:InvalidReduction'));
end

if ~isequal(r, floor(r)) || min(r)<1
  error(message('MATLAB:reducevolume:ReductionMustBeInt'));
end
...
if length(newsz)<3 || min(newsz)<2
  error(message('MATLAB:reducevolume:ReductionTooHigh'));
end
if length(sz)>3
...
elseif nin==5       % reducevolume(x,y,z,v,r)
  ...
  error(message('MATLAB:reducevolume:WrongNumberInputs')); 
end
