...
%HISTCOUNTS  Histogram Bin Counts of datetimes.
%   [N,EDGES] = HISTCOUNTS(X) partitions the values in datetime array X
...
%
%   [N,EDGES] = HISTCOUNTS(X,EDGES), where EDGES is a datetime vector in
%   sorted order, specifies the edges of the bins.
%
%   [N,EDGES] = HISTCOUNTS(...,'BinWidth',BW) uses bins of width BW, which is
%   a scalar duration or calendarDuration. To prevent from accidentally
...
%   are needed, HISTCOUNTS uses a larger bin width corresponding 
...
%   be the following time units: 'second', 'minute', 'hour', 'day', 'week',
%   'month', 'quarter', 'year', 'decade', or 'century', which places bin
%   edges at boundaries of the time unit. BM can also be:
...
%
%   See also HISTOGRAM, DATETIME/DISCRETIZE
