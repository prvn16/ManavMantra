...
%
%   Y = MOVMIN(...,MISSING) specifies how NaN (Not-a-Number) values are
...
%                        also NaN.
%
%   Y = MOVMIN(...,'Endpoints',ENDPT) controls how the minimum is
...
%   same size as X.
%
%   Y = MOVMIN(X,K,...,'SamplePoints',T) computes a centered moving minimum
%   of X using a window of length K with respect to the sample points T.
%   Each element Y(i) is the local minimum of X(IDX), where IDX is the set
%   of indices such that (T(i)-K/2) <= T(IDX) < (T(i)+K/2). Each window has
%   a closed left end and an open right end.
%
%   Y = MOVMIN(X,[NB NF],...'SamplePoints',T) computes a moving minimum of
...
%   specify time stamps for the data. By default, MOVMIN uses data sampled
...
%
%   Example: Compute a 5-hour centered moving minimum of non-uniformly
%   spaced data.
%       x = [4 3 1 -2 -4 -1 0 2 -2 3];
%       t = datetime('now') + hours([0 2 5:12]);
%       yd = movmin(x,hours(5),'SamplePoints',t);
%
