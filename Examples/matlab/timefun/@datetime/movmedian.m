...
%
%   Y = MOVMEDIAN(...,MISSING) specifies how NaN (Not-a-Number) values are
...
%
%   Y = MOVMEDIAN(...,'Endpoints',ENDPT) controls how the median is
...
%   same size as X.
%
%   Y = MOVMEDIAN(X,K,...,'SamplePoints',T) computes a centered moving
%   median of X using a window of length K with respect to the sample
%   points T. Each element Y(i) is the local median of X(IDX), where IDX is
%   the set of indices such that (T(i)-K/2) <= T(IDX) < (T(i)+K/2). Each
%   window has a closed left end and an open right end.
%
%   Y = MOVMEDIAN(X,[NB NF],...'SamplePoints',T) computes a moving median
%   of X using a window of width (NF-NB) with respect to the sample points
%   T. Each element Y(i) is the local median of X(IDX), where IDX is the
%   set of indices such that (T(i)-NB) <= T(IDX) <= (T(i)+NF). Both ends
%   for each window are closed.
%
...
%   specify time stamps for the data. By default, MOVMEDIAN uses data
%   sampled uniformly at points T = [1 2 3 ... ].
%
...
%
%   Example: Compute a 5-hour centered moving median of non-uniformly
%   spaced data.
%       x = [4 8 6 -1 -2 -3 -1 3 4 5];
%       t = datetime('now') + hours([0 2 5:12]);
%       yd = movmedian(x,hours(5),'SamplePoints',t);
%
