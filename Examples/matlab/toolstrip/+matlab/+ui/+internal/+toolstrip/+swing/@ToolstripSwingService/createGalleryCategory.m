function createGalleryCategory(this, widget_node)
    ...
    jtitle = widget_node.getProperty('title');
    jh = javaObjectEDT('com.mathworks.toolstrip.components.gallery.model.Category',name,jtitle);
    ...
    % to do: add callback to category moved event
...
function propertySetCallback(~, data, this, widget_id)
    ...
    if isa(originator, 'java.util.HashMap') && strcmp(originator.get('source'),'MCOS')
        ...
        switch structure.key
            case 'title'
                warning('Gallery category title cannot be changed after rendering!')
        end
    end
end
