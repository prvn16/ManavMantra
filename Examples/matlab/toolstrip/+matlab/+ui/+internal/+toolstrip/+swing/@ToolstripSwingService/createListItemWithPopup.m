function createListItemWithPopup(this, widget_node)
    ...
    popupId = char(action_node.getProperty('popupId'));
    if isempty(popupId)
        error('you must specify popup list before rendering list item with popup!')
    else
        jpopup = this.Registry.getWidgetById(popupId);    
    end
    jh = javaMethodEDT('newSubList','com.mathworks.toolstrip.components.popups.ListItem',name,'title',[],jpopup);
