function loadCustomshortcuts(h)
%LOADCUSTOMSHORTCUTS Loads shortcuts saved with the model.
...
try
    ...
    if ~isempty(buttonInfo)
        ...
        % been saved yet.
        if h.ButtonActionMap.isKey(h.getFPTRoot.getDAObject.Handle)
            shortcutButtons = h.ButtonActionMap.getDataByKey(h.getFPTRoot.getDAObject.Handle);
            defaultButtons = h.getDefaultShortcutButtons;
            if isequal(shortcutButtons, defaultButtons)
                h.setShortcutButtonListForModel(bName);
            end
        else
            h.setShortcutButtonListForModel(bName);
        end
    ...
    for i = 2:length(shortcutsArray)
        ...
        mdlSettingMap = getGlobalSettingMapForShortcut(h,sName);
        ...
        for m = 6:incrLoopIndex:length(shortcutInfo)
                ...
                if isempty(curBlkSID)
                    ...
                    if incrLoopIndex == 5
                        ...
                        if isempty(topModelPath)
                            ...
                            blkSID = createSIDFromPathTrace(topModelPath, bd);
                        end
                ...
                elseif isempty(strfind(curBlkSID, ':'))
                    ...
                    if incrLoopIndex == 5
                        ...
                        if isempty(topModelPath)
                            ...
                            mdlName = createSIDFromPathTrace(topModelPath, bd);
            ...
            blkSettingMap = getSystemSettingMapForShortcut(h,blkSID,shortcutName);
...
%--------------------------------------------------------------------------
function blkSID = createSIDFromPathTrace(topModelPathTrace, bd)
% Get the model name from the top modle trace stored for referenced models.

topModelName = bd.getFullName;
