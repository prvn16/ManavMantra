function intPoints = NonMaxSuppression_gpu(intImage, responseMap) %#codegen
...
for o = coder.unroll(1:5)
    
    for i = coder.unroll(1:2)
        ...
        for c = 0:i_width-1
            ...
            for r = 0:i_height-1
                if(keyMatrix(r+1,c+1))
                    ctrVal = coder.nullcopy(int32(0));
                    ctrVal = coder.ceval('-gpudevicefcn', 'atomicAdd', coder.ref(ctr), int32(1));
                    if (ctrVal <= 2000)
                    ...
                    intPoints(ctrVal).x = single((c + xc) * t_step);
                    intPoints(ctrVal).y = single((r + xr) * t_step);
                    intPoints(ctrVal).scale = single((0.1333) * (m_filter + xi * filterStep));
                    intPoints(ctrVal).laplacian = int32(getLaplacian(m_laplacian, m_width, r, c, t_width));
                    intPoints(ctrVal).orientation = single(0);
                    end
                end
            end
        end
