...
%   ptable = powerbw(fb);
    ...
    properties (SetAccess = private)
        ...
        %   less than or equal to floor(log2(SignalLength)).
        Level
    ...
    methods (Access = public)
        ...
        function bptable = powerbw(self)
            ...
            [bwpsi,fhpsi,flpsi,pwrpsi,totpwrpsi] = ...
                halfpowerbandwidth(self,SxxPsi',[]);
            ...
            [bwphi,fhphi,flphi,pwrphi,totpwrphi] = ...
                halfpowerbandwidth(self,SxxPhi',[]);
            ...
            for kk = 1:self.Level
                [~,~,~,pwrspi,totpwrpsi] = ...
                    halfpowerbandwidth(self,SxxPsi(kk,:)',self.PassbandFrequencies(kk,:));
            ...
            for kk = 1:self.Level
                ...
                [~,~,~,pwrphi,totpwrphi] = ...
                    halfpowerbandwidth(self,SxxPhi(kk,:)',frange);
        ...
        function tf = isOrthogonal(self,tol)
            ...
            % Expecting real-valued FIR filters. For an orthogonal wavelet
            % the analysis filters should be time-reversed versions of the
            % synthesis filters.
            tfrev = @(x,y)isequal(x,flipud(y));
            tflow = tfrev(LoD,LoR);
            tfhigh = tfrev(HiD,HiR);

            % The lengths will always be the same for the synthesis
            % filters.

            internalCheck = wavelet.internal.CheckFilter(LoD,HiD, tol);

            Lscaling = length(LoD);
            % Check DFT condition
            % We can use either length, they must be the same.
            npad = nextpow2(Lscaling);
            LoDdft = fft(LoD,2^npad);
            % There is a phase factor we are ignoring here by just taking
            % the conjugate of the DFT. This is ok because we have already
            % checked that the synthesis filters are time-reversed versions
            % of the analysis filters.
            LoRdft = conj(LoDdft);
            HiDdft = fft(HiD,2^npad);
            HiRdft = conj(HiDdft);
            N = 2^npad/2;
            Lodft = LoDdft.*LoRdft;
            Lotest = Lodft(1:N)+Lodft(N+1:end);
            if any(abs(Lotest-2) > tol)
                zeroevenlagsLo = false;
            else
                zeroevenlagsLo = true;
            end
            Hidft = HiDdft.*HiRdft;
            Hitest = Hidft(1:N)+Hidft(N+1:end);
            if any(abs(Hitest-2)>tol)
                zeroevenlagsHi = false;
            else
                zeroevenlagsHi = true;
            end
            zeroevenlags = all([zeroevenlagsLo zeroevenlagsHi]);
            tf = all([internalCheck tflow tfhigh zeroevenlags]);
        end
        ...
        function tf = isBiorthogonal(self,tol)
            ...
            % Expecting real-valued FIR filters
            tfrev = @(x,y)isequal(x,flipud(y));
            % For biorthogonal filters, the filters are NOT be time
            % reverses of each other. They are not self-dual.
            tflow = ~tfrev(LoD,LoR);
            tfhigh = ~tfrev(HiD,HiR);
            Lscaling = length(LoD);
            Lwavelet = length(HiD);
            evenlengthLo = ~logical(rem(Lscaling,2));
            evenlengthHi = ~logical(rem(Lwavelet,2));
            ...
            % Both scaling filters should sum to sqrt(2) and both wavelet
            % filters should sum to zero.
            if abs(sumLoD-sqrt(2)) > tol || abs(sumLoR-sqrt(2)) > tol ...
                    || sumHiD > tol || sumHiR > tol
                sumfilters = false;
            else
                sumfilters = true;
            end

            % Check zeros in complementary filters
            zerotol = 1e-3;
            LoDzeros = roots(LoD);
            HiDzeros = roots(HiD);
            LoRzeros = roots(LoR);
            HiRzeros = roots(HiR);
            tfzero1 = isequal(find(abs(LoDzeros-1)<zerotol),...
                find(abs(HiRzeros+1)<zerotol));
            tfzero2 = isequal(find(abs(LoRzeros-1)<zerotol),...
                find(abs(HiDzeros+1)<zerotol));

            tf = all([evenlength equallen sumfilters tfzero1 tfzero2...
                tfhigh tflow]);
        end
    ...
    methods (Access = private, Hidden)
        ...
        function self = setProperties(self,varargin)
            ...
            lvalid = @(l)validateattributes(l,{'numeric'}, ...
                {'scalar', 'nonnegative', 'finite'}, 'DWTFILTERBANK', ...
                'Level');
            ...
            if isempty(self.Level)
                self.Level = fix(log2(self.SignalLength/(L-1)));
            elseif self.Level > maxlev
        ...
        function [bw,fhi,flo,pwr,totpwr] = halfpowerbandwidth(self,Sxx,freqrange)
            ...
            [bw,fhi,flo,pwr,totpwr] = wavelet.internal.computePowerBW(Pxx,F,freqrange,R);
