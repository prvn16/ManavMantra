...
% from a MATLAB file and deploy the application on an ARM target. 
%
% The example reads video frames from a webcam and detects faces in each of
% the frames using the Viola-Jones face detection algorithm.  The detected
% faces are displayed with bounding boxes. The webcam function, from
% 'MATLAB Support Package for USB Webcams', and the DeployableVideoPlayer
% object, from the Computer Vision System toolbox(TM), are used for the
% simulation on the MATLAB host. The two functions do not support the ARM
% target, so OpenCV-based webcam reader and video viewer functions are used
% for deployment.
%
...
% resides in a function called <matlab:edit('faceDetectionARMKernel.m')
% faceDetectionARMKernel.m>. The function takes an image from a webcam, as
% the input. The function outputs the image with a bounding box around the
% detected faces. The output image will be displayed on video viewer
% window. To learn how to modify the MATLAB code to make it compatible for
% code generation, you can look at example
...
% faceDetectionARMMain.c file. This main function in this file performs the
% following tasks:
% 
...
% faceDetectionARMMain.c file is implemented in faceDetectionARMMain.m
...
% the simulation on the MATLAB host by typing faceDetectionARMMain at the
...
bInfo = load(fullfile(codegenOutDir,'codegen','exe','faceDetectionARMKernel','buildInfo.mat'));
...
% Unzip faceDetectionARMKernel.zip into a folder named FaceDetectionARM.
...
% The Makefile, faceDetectionARMMakefile, provided in this example is
...
% your OpenCV libraries located in /home/pi/opencv-3.1.0/lib. You can
% update the Makefile based on your target configuration. Copy the Makefile
% to the project folder.
copyfile(fullfile(helperFilePath, 'faceDetectionARMMakefile'), packngoDir);
% Also move the file containing the main function in the top level folder.
copyfile(fullfile(helperFilePath, 'faceDetectionARMMain.c'), packngoDir);
...
% Transfer your project folder named FaceDetectionARM to your ARM target
...
% the host to the _/home/pi/<user_name>/FaceDetectionARM_ folder on
% Raspberry Pi.
...
% /home/pi/<user_name>/FaceDetectionARM. Build the executable using the
% following command: 
%
% _make -f faceDetectionARMMakefile_  
%
...
%% Appendix - Helper Functions 
    ...
    function codegenArgs = createCodegenArgs(folderForMainC)
        ...
        mainCFile = fullfile(folderForMainC,'faceDetectionARMMain.c');
