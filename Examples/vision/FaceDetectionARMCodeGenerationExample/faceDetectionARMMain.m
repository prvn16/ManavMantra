...
% detection algorthm and finally displays the frame with bounding box
% around the detected face. This function uses webcam function from 'MATLAB
...
% mimicked in faceDetectionARMMain.c with some modifications. One
% difference is that- in this function the loop terminates when the video
% player window is closed, whereas in faceDetectionARMMain.c, the loop
...
while cont
    inRGB = snapshot(cam);
    outRGB = faceDetectionARMKernel(inRGB);
...
coder.internal.errorIf(~isempty(coder.target), ...
                       'vision:insertText:FontFileMissing');
