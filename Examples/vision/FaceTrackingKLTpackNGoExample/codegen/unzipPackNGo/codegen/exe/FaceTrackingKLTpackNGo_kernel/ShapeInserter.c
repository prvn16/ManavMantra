 ...
 * Return Type  : void
 */
void ShapeInserter_outputImpl(visioncodegen_ShapeInserter *obj, float
 ...
 * Return Type  : void
 */
void b_ShapeInserter_outputImpl(visioncodegen_ShapeInserter_1 *obj, float
  varargin_1[921600], const int varargin_2_data[], const int varargin_2_size[1],
  const float varargin_3[3])
{
  vision_ShapeInserter_6 *b_obj;
  int numSubShape;
  int colBoundary;
  int ii;
  int len;
              ...
              if ((loopEndIdx == 0) || (!isMore)) {
                while (y2 <= halfLineWidth) {
                  ...
                  for (loopEndIdx = 0; loopEndIdx < 3; loopEndIdx++) {
                    if (y1_M_y2 == 64) {
                      varargin_1[numEdge] += varargin_3[x1_M_x2] -
                        varargin_1[numEdge];
                    } else {
                      varargin_1[numEdge] += (varargin_3[x1_M_x2] -
                        varargin_1[numEdge]) * (float)y1_M_y2 / 64.0F;
                    }
            ...
            for (loopEndIdx = 0; loopEndIdx < 3; loopEndIdx++) {
              ...
              for (row = ii; row <= idx; row++) {
                ...
                if (subPixCount == 64) {
                  ...
                  varargin_1[numEdge] += (varargin_3[x1_M_x2] -
                    varargin_1[numEdge]) * (float)subPixCount / 64.0F;
                }
