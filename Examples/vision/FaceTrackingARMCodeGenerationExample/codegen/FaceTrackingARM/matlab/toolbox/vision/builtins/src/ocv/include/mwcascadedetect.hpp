//#pragma once
#ifndef _MWCASCADEDETECT_HPP_
#define _MWCASCADEDETECT_HPP_
...
//#include <stdint.h>
#include "mwobjdetect.hpp" // for MWCascadeClassifier
...
//----------------------------------------------  HaarEvaluator ---------------------------------------
class HaarEvaluator : public MWFeatureEvaluator
{
public:
    struct MWFeature
    {
    ...
    virtual int getFeatureType() const { return MWFeatureEvaluator::HAAR; }
...
protected:
    Size origWinSize;
    Ptr<vector<MWFeature> > features;
    MWFeature* featuresPtr; // optimization
...
inline HaarEvaluator::MWFeature :: MWFeature()
{
    tilted = false;
...
inline float HaarEvaluator::MWFeature :: calc( int _offset ) const
{
...
inline void HaarEvaluator::MWFeature :: updatePtrs( const Mat& _sum )
{
...
class LBPEvaluator : public MWFeatureEvaluator
{
public:
    struct MWFeature
    {
        MWFeature();
        MWFeature( int x, int y, int _block_w, int _block_h  ) :
    ...
    virtual bool read( const FileNode& node );
    virtual Ptr<MWFeatureEvaluator> clone() const;
    virtual int getFeatureType() const { return MWFeatureEvaluator::LBP; }
...
inline LBPEvaluator::MWFeature :: MWFeature()
{
    rect = Rect();
    for( int i = 0; i < 16; i++ )
        p[i] = 0;
...
inline int LBPEvaluator::MWFeature :: calc( int _offset ) const
{
...
inline void LBPEvaluator::MWFeature :: updatePtrs( const Mat& _sum )
{
    const int* ptr = (const int*)_sum.data;
    size_t step = _sum.step/sizeof(ptr[0]);
    Rect tr = rect;
...
class HOGEvaluator : public MWFeatureEvaluator
{
public:
    struct MWFeature
    {
    ...
    virtual bool read( const FileNode& node );
    virtual Ptr<MWFeatureEvaluator> clone() const;
    virtual int getFeatureType() const { return MWFeatureEvaluator::HOG; }
...
private:
    ...
    Size origWinSize;
    Ptr<vector<MWFeature> > features;
    MWFeature* featuresPtr;
    vector<Mat> hist;
    Mat normSum;
    int offset;
};

inline HOGEvaluator::MWFeature :: MWFeature()
{
...
inline float HOGEvaluator::MWFeature :: calc( int _offset ) const
{
...
inline void HOGEvaluator::MWFeature :: updatePtrs( const vector<Mat> &_hist, const Mat &_normSum )
{
...
inline int predictOrdered( MWCascadeClassifier& cascade, Ptr<MWFeatureEvaluator> &_featureEvaluator, double& sum )
{
    int nstages = (int)cascade.data.stages.size();
    int nodeOfs = 0, leafOfs = 0;
    FEval& featureEvaluator = (FEval&)*_featureEvaluator;
    float* cascadeLeaves = &cascade.data.leaves[0];
    MWCascadeClassifier::Data::DTreeNode* cascadeNodes = &cascade.data.nodes[0];
    MWCascadeClassifier::Data::DTree* cascadeWeaks = &cascade.data.classifiers[0];
    MWCascadeClassifier::Data::Stage* cascadeStages = &cascade.data.stages[0];

    for( int si = 0; si < nstages; si++ )
    {
        MWCascadeClassifier::Data::Stage& stage = cascadeStages[si];
        int wi, ntrees = stage.ntrees;
        sum = 0;

        for( wi = 0; wi < ntrees; wi++ )
        {
            MWCascadeClassifier::Data::DTree& weak = cascadeWeaks[stage.first + wi];
            int idx = 0, root = nodeOfs;

            do
            {
                MWCascadeClassifier::Data::DTreeNode& node = cascadeNodes[root + idx];
...
template<class FEval>
inline int predictCategorical( MWCascadeClassifier& cascade, Ptr<MWFeatureEvaluator> &_featureEvaluator, double& sum )
{
    int nstages = (int)cascade.data.stages.size();
    int nodeOfs = 0, leafOfs = 0;
    FEval& featureEvaluator = (FEval&)*_featureEvaluator;
...
template<class FEval>
inline int predictOrderedStump( MWCascadeClassifier& cascade, Ptr<MWFeatureEvaluator> &_featureEvaluator, double& sum )
{
    int nodeOfs = 0, leafOfs = 0;
    FEval& featureEvaluator = (FEval&)*_featureEvaluator;
    float* cascadeLeaves = &cascade.data.leaves[0];
    ...
    {
        MWCascadeClassifier::Data::Stage& stage = cascadeStages[stageIdx];
        sum = 0.0;

        int ntrees = stage.ntrees;
        for( int i = 0; i < ntrees; i++, nodeOfs++, leafOfs+= 2 )
        {
            MWCascadeClassifier::Data::DTreeNode& node = cascadeNodes[nodeOfs];
...
template<class FEval>
inline int predictCategoricalStump( MWCascadeClassifier& cascade, Ptr<MWFeatureEvaluator> &_featureEvaluator, double& sum )
{
    int nstages = (int)cascade.data.stages.size();
    int nodeOfs = 0, leafOfs = 0;
    FEval& featureEvaluator = (FEval&)*_featureEvaluator;
